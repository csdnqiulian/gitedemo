...
public class SimpleConnetionPool {
	...
	static private long m_lastClearClosedConnection = System
			.currentTimeMillis();
	public static long CHECK_CLOSED_CONNECTION_TIME = 4 * 60 * 60 * 1000;// 4hours
	...
	private static void clearClosedConnection() {
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				if (DEBUG) {
					System.out
							.println("connectionisclosed,thisconnectioninitialStackTrace");
					wrapper.debugInfo.printStackTrace();
				}
			}
		}
...
class ConnectionWrapper implements InvocationHandler {
	...
	Throwable debugInfo = new Throwable("Connectioninitialstatement");
